...
zeroBtn.addEventListener('click', () => {
    if (sensorStatusEl.textContent === "Connected") {
        ...
        const zeroValue = parseFloat(sensorValueEl.textContent);
        sendValuesToServer();
...
calibrateBtn.addEventListener('click', () => {
    if (sensorStatusEl.textContent === "Connected") {
        // Capture the current sensor value displayed on the website
        const calibrateValue = parseFloat(sensorValueEl.textContent);
        sendValuesToServer();

        //console.log("Calibrate value captured:", currentValue);

        // Optional: show alert
        alert("Sensor calibration value captured!");
    } else {
        alert("Sensor not connected!");
    }
});
...
function sendValuesToServer() {
    fetch("/update-values", {
        ...
        body: JSON.stringify({
            ...
            zero: parseInt(zeroValue),
            calib: parseInt(calibrateValue)
        })
    });
}
